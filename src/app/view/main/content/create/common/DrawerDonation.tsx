...
interface DrawerDonationProps {
  ...
  onClose: () => void;
  onSend?: (price: string) => void;
}

const DrawerDonation: React.FC<DrawerDonationProps> = React.memo(
  ({isOpen, sponsoredName, giveToPDId, onClose, onSend}) => {
        ...
        >
          {sponsoredName && (
            <div className={styles.donationUserInfoText}>
              {formatText(getLocalizedText('shared010_desc_002'), [
                dataProfile?.currentProfile?.name || '',
                sponsoredName,
              ])}
            </div>
          )}
