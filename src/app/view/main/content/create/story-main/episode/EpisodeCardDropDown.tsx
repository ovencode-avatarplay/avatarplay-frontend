...
import {EpisodeInfo, setSelectedEpisodeIdx, updateEpisodeInfo} from '@/redux-store/slices/StoryInfo';
import {useDispatch, useSelector} from 'react-redux';
import {LineArrowSwap, LineCopy, LineDelete, LineEdit, LinePreview} from '@ui/Icons';
import {ChapterInfo, duplicateEpisode, removeEpisode} from '@/redux-store/slices/StoryInfo';
...
const EpisodeCardDropDown: React.FC<EpisodeCardDropDownProps> = ({save, episodeInfo, open, close}) => {
  ...
  const chapterInfo = useSelector((state: RootState) => {
    return state.story.curEditingStoryInfo.chapterInfoList.find(chapter =>
  ...
  const handleSetEpisodeNameComplete = (name: string): boolean => {
    try {
      ...
      const allEpisodes = state.story.curEditingStoryInfo.chapterInfoList.flatMap(
