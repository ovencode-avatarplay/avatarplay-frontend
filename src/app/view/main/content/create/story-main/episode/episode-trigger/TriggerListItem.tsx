...
import {moveTriggerToEpisode} from '@/redux-store/slices/StoryInfo';
...
const TriggerListItem: React.FC<TriggerListItemProps> = ({handleToggle, isSelected, index}) => {
  ...
  const handleConfirm = (chapterIdx: number, episodeIdx: number) => {
    ...
    dispatch(
      moveTriggerToEpisode({
        ...
        targetEpisodeId: editingStoryInfo.chapterInfoList[chapterIdx].episodeInfoList[episodeIdx].id, // 대상 에피소드 ID
      }),
    );
  };
  ...
  const handleOnFileSelect = async (files: File[]) => {
    try {
      ...
      if (state === UploadMediaState.Story) {
        req.fileList = files;
      } else {
        req.fileList = files;
      }
