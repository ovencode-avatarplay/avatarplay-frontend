...
import {setCurrentEpisodeBackgroundImage} from '@/redux-store/slices/StoryInfo';
...
const EpisodeUploadCharacter: React.FC = () => {
  ...
  const GetImageUrlByFile = async (image: File) => {
    ...
    try {
      ...
      const req: MediaUploadReq = {
        mediaState: UploadMediaState.Character, // 적절한 MediaState 설정
        fileList: [image],
      };
      ...
      if (response?.data) {
        ...
        console.log('Additional image URLs:', response.data.mediaUploadInfoList); // 추가 이미지 URL 출력
      } else {
  ...
  useEffect(() => {
    if (editedEpisodeInfo?.characterInfo) setImagePreview(editedEpisodeInfo?.characterInfo?.mainImageUrl);
  }, [editedEpisodeInfo]);
