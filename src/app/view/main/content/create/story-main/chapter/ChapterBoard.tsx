...
// Slice
import {setSelectedChapterIdx, setSelectedEpisodeIdx, ChapterInfo, EpisodeInfo} from '@/redux-store/slices/StoryInfo';

// Components
import CreateDrawerHeader from '@/components/create/CreateDrawerHeader';

// Data
import emptyData from '@/data/create/empty-story-info-data.json';
...
const ChapterBoard: React.FC<Props> = ({
  ...
  const editingStoryInfo = useSelector((state: RootState) => state.story.curEditingStoryInfo);
  ...
  useEffect(() => {
    if (open) {
      if (
        ...
        editingStoryInfo.chapterInfoList[selectedChapterIdx].episodeInfoList[selectedEpisodeIdx]
      ) {
        // dispatch();
        ...
        // dispatch(setCurrentEpisodeInfo(editingStoryInfo.chapterInfoList[0].episodeInfoList[0]));
      }
    }
  ...
  const handleDuplicateChapter = () => {
    if (selectedChapterIdx < 0 || selectedChapterIdx >= editingStoryInfo.chapterInfoList.length) {
      console.error('Invalid chapter index.');
      return;
    }

    const selectedChapter = editingStoryInfo.chapterInfoList[selectedChapterIdx];
    const newChapterId = getMinChapterId(editingStoryInfo.chapterInfoList);
