...
import styles from './StoryImageUpload.module.css';
...
const StoryImageUpload: React.FC<Props> = ({
  uploadImageState: uploadImageOpen,
  initImage,
  onClickUploadImage,
  onCloseUploadImage,
}) => {
  const editedStoryInfo = useSelector((state: RootState) => state.story);
  const thumbnail = useSelector((state: RootState) => state.story.curEditingStoryInfo.publishInfo.thumbnail);
  ...
  useEffect(() => {
    setImagePreview(editedStoryInfo?.curEditingStoryInfo.publishInfo.thumbnail);
  }, [editedStoryInfo, thumbnail]);
  return (
    <>
