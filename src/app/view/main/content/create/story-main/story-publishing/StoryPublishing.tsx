...
import StoryImageUpload from './StoryImageUploader';
...
const StoryPublishing: React.FC<Props> = ({open, onClose, onPublish, LLMOpen, setLLMOpen, onLLMClose}) => {
  const dispatch = useDispatch();
  const {thumbnail, storyDescription, authorComment, visibilityType, monetization, nsfw, selectTagList} = useSelector(
  ...
  const handleIntroductionChange = (event: React.ChangeEvent<HTMLTextAreaElement>) => {
    dispatch(setStoryDescription(event.target.value));
  };
  ...
  return (
    ...
    >
      <div className={styles.modalContainer}>
        ...
        {/* EpisodeLLMSetup 모달 */}
        <StoryLLMSetup open={LLMOpen} onClose={onLLMClose} />
      </div>
    </Modal>
  );
};

export default StoryPublishing;
