...
// 사용자 정보를 업데이트하는 함수
// export function updateUserInfo(userId: number, newFullName: string, newRole: string): void {
//   // 주어진 ID를 가진 사용자의 인덱스를 찾습니다.
//   console.log( "채팅 타입 데이터 수정됨 ");
//   const userIndex = db.contacts.findIndex(contact => contact.id === userId);
//   console.log( userIndex, newFullName, newRole);
//   console.log(db.contacts[userIndex]);
//   // 사용자가 존재하는지 확인합니다.
//   if (userIndex !== -1) {
//     // 사용자의 이름과 역할을 업데이트합니다.
//     try{
//       db.contacts[userIndex].fullName = newFullName;
//       db.contacts[userIndex].role = newRole;
